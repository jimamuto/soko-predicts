...
import { 
  TrendingUp, TrendingDown, Minus, Calendar, MapPin, Package, Filter, 
  BarChart3, Download, RefreshCw, ArrowRight, Zap 
} from "lucide-react";
...
export default function PredictionsHistory() {
  ...
  const filteredData =
    filter === "all" ? data : data.filter((p) => p.trend === filter);
  ...
  const stats = {
    ...
    avgConfidence:
      Math.round((data.reduce((acc, p) => acc + p.confidenceScore, 0) / data.length) * 100) || 0,
  };
      ...
      <div className="bg-white border-b border-gray-200 py-12">
        <div className="max-w-6xl mx-auto px-6 text-center">
          <motion.div
            ...
            className="inline-flex items-center space-x-2 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full px-4 py-2 mb-6 shadow-md"
          >
      ...
      {data.length > 0 && (
        <div className="max-w-6xl mx-auto px-6 -mt-8 mb-8">
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="bg-white border border-gray-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition-all">
              <div className="text-2xl font-bold text-gray-900">{stats.total}</div>
              <div className="text-sm text-gray-600">Total Predictions</div>
            </div>
            <div className="bg-green-50 border border-green-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition-all">
              <div className="text-2xl font-bold text-green-600">{stats.rising}</div>
              <div className="text-sm text-green-700">Rising Trends</div>
            </div>
            <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition-all">
              <div className="text-2xl font-bold text-red-600">{stats.falling}</div>
              <div className="text-sm text-red-700">Falling Trends</div>
            </div>
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition-all">
              <div className="text-2xl font-bold text-gray-600">{stats.stable}</div>
              <div className="text-sm text-gray-700">Stable Trends</div>
            </div>
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition-all">
            ...
            {sortedData.map((prediction, index) => (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div
                    className={`flex items-center gap-1 px-3 py-1 rounded-full ${getTrendColor(
                      prediction.trend
                    )}`}
                  >
                ...
                <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-200">
                  ...
                  <button className="bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-800 transition-all text-sm flex items-center gap-2">
